...
namespace inVtero.net.Support
{
    public static class Capstone
    {
        ...
        public enum x86_insn_group : int
        {
        ...
        public static cs[] Dissassemble(Byte[] Input, int Length, ulong VA = 0, bool Details = false)
        {
            ...
            var insnHandle = new IntPtr();
            var disHandle = new IntPtr();

            cs_open(cs_arch.CS_ARCH_X86, cs_mode.CS_MODE_64, ref disHandle);
            // Intel only for now
            cs_option(disHandle, 1, 1);
            // Add detail mode
            if (Details)
                cs_option(disHandle, 2, 3);

            // skipdata
            cs_option(disHandle, 5, 3);

            var count = cs_disasm(disHandle, Input, Length, VA, 0, ref insnHandle);
            var insn_size = Marshal.SizeOf<cs_insn>();
            var BuffOffset = insnHandle.ToInt64();

            for (var curr = 0U; curr < count; curr++)
            {
                var InsnPointer = new IntPtr(BuffOffset);
                var cs = new cs() { insn = Marshal.PtrToStructure<cs_insn>(InsnPointer) };
                if (Details && cs.insn.id != 0)
                    cs.detail = Marshal.PtrToStructure<cs_detail>(cs.insn.detail);

                rv.Add(cs);
                BuffOffset += insn_size;
            }
            cs_close(ref disHandle);
            cs_free(insnHandle, count);
            return rv.ToArray();
        }
